...
public class DBHandler {
    ...
    /**
     * This method is called by {@link #checkAndAddUser(User, String, Context, ProgressBar)} to add an entry to the DB
     * after determining that the username does not exist.
     * @param doc Document generated by {@link #checkAndAddUser(User, String, Context, ProgressBar)} that will store the user info.
     * @param user A {@link User} object that contains all the user information.
     * @param password A {@link String} object that is the password (not stored in {@link User} for security.
     * @param context Activity that the error toast will appear in.
     * @param bar A progress bar that disappears once database fetching is done.
     */
    private void newUser(DocumentReference doc, User user, String password, Context context, ProgressBar bar) {
        HashMap<String, Object> data = new HashMap();
        data.put("first_name", user.getDisplayName());
    ...
    /**
     * This method will check if the chosen username already exists in the database. If it does, an error toast will be returned.
     * Otherwise, a new user entry will be added to the database and the user will get logged in automatically with that username.
     * @param user A {@link User} object that contains all the user information.
     * @param password A {@link String} object that is the password (not stored in {@link User} for security.
     * @param context Activity that the error toast will appear in.
     * @param bar A progress bar that disappears once database fetching is done.
